...
- **Channel keyword scan over-triggers (surfaced in theline smoke test, 2026-04-14):** activate logs often mention channels historically ("pivoted from HeyReach/Smartlead to Gmail") which causes the full-body keyword scan to match multiple classes and fall back to user-prompt. Fix: before the full-body scan, look for structured headers (`**Mode:**`, `**Channel decision**`, `Channel:`) and scan only the text after the first such header. Falls back to full-body scan if no structured header found. Small patch — defer unless friction accumulates.
- **Channel taxonomy conflates medium with mode (surfaced in theline smoke test, 2026-04-14):** current vocabulary is `linkedin` / `email` / `gmail`, but `gmail` is really "email delivered as drafts" — both `email` and `gmail` are the same medium. Richard's natural phrasing ("email in draft") reflects the mismatch. Consider v2 schema change: split into `channel` (`linkedin` / `email`) + `mode` (`live` / `draft`). Affects results.csv schema, `/capture-results`, CLAUDE.md, and any future per-channel `/critique` slicing. Not a hot fix — tackle when the schema is touched for another reason.
